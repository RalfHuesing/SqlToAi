...
invoked_as: ".agents/workflows/konzept-workflow.md <task-dir>"
produces_input_for: .agents/workflows/initial-workflow.md
---
...
> `.agents/workflows/konzept-workflow.md tasks/tetris-mobile`
...
`.agents/workflows/task-loop.md` §6 (Mindestanforderungen) erfüllt —
danach kann `.agents/workflows/initial-workflow.md` direkt darauf
aufsetzen.

Diese Datei ist bewusst **tool-agnostisch und projekt-unabhängig**
formuliert — unverändert kopierbar in jedes andere Projekt, wie
`task-loop.md`, `.agents/skills/**` und `.agents/templates/**` auch.

## Grundprinzip: Dialog statt Autonomie

Das unterscheidet diesen Workflow fundamental von `task-loop.md`: Dort
...
- Hat der Nutzer im Aufruf-Prompt schon eine Roh-Idee mitgegeben: die
  ...
  `.agents/templates/konzept.md`), `status: draft`.
...
- Lies `.agents/rules/**`, `README.md`, `docs/**` — was immer davon
  existiert. Das ist dieselbe Anker-Grundlage wie beim Planer in
  `task-loop.md`.
...
Ziel-Struktur = alle Abschnitte aus `.agents/templates/konzept.md`. Geh
für jeden Abschnitt durch: schon ausreichend konkret, oder noch offen?
...
- Kurze Zusammenfassung + Hinweis: *„Bereit für
  `.agents/workflows/initial-workflow.md <task-dir>`."*
...
- **Keine Implementierungsdetails vorwegnehmen**, die eigentlich der
  Planer in `task-loop.md` treffen sollte (konkrete Dateiaufteilung,
  Klassen-/Funktionsnamen, Zeilen-genaue Änderungen) — hier geht es um
  das Konzept, nicht den Umsetzungsplan.
