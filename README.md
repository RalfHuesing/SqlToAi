...
* 🔑 **Reversible, Searchable Tokenization (optional):** A global mode switch, just like `DefaultMode` — when enabled, every column that would be anonymized gets a deterministic, keyed token instead of a scramble/hash mask, automatically, with zero column-by-column configuration (built for the reality of dozens of databases and hundreds of tables, where a per-column allowlist would never stay maintained). The server resolves that exact token back to the real value inside string literals before a later query executes — so `WHERE`, `JOIN`, `LIKE`, and range comparisons all keep working across tables, without the AI ever learning the value itself. Which columns get anonymized at all is unaffected — that's still entirely up to `ExcludedColumns`/`ExclusionTableName`/`AnonymizationRules`.
...
| `Anonymizer` | Master switch (`Enabled`), the algorithm (`DefaultMode`: `ScramblePattern` or `Hash`), the list of column-name patterns that must NOT be anonymized (`ExcludedColumns`), and the optional `Tokenization` sub-section below. |
| `Anonymizer.Tokenization` | Optional global mode switch (`Enabled`, `Secret`, `Prefix`/`Suffix`) that replaces `DefaultMode` masking with reversible tokens for every anonymized column — no per-column list. See [mcp-specification.md](docs/mcp-specification.md#f-reversible-durchsuchbare-tokenisierung-anonymizertokenization-optional). |
